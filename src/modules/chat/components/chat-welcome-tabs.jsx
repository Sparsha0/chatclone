...
const CHAT_TAB_MESSAGE =[
    {
        tabName:"Create",
        icon:<Sparkles className="h-4 w-4"></Sparkles>,
        messages:[
    ...
    {
        tabName: "Learn",
        icon: <GraduationCap className="h-4 w-4"></GraduationCap>,
        messages:[
            "Beginner's guide to TypeScript",
            "Explainthe the CAP theorem in distributed systems",
            ...
            <div className="w-full max-w-3xl space-y-8">
                ...
                <div className="flex flex-wrap gap-2 w-full">
                    {CHAT_TAB_MESSAGE.map((tab, index)=>(
                        ...
                        className="w-[110] justify-start">
                            {tab.icon}
                            <span className="ml-2">{tab.tabName}</span>
                        </Button>
                    ))}
                </div>
                <div className="space-y-3 w-full min-h-[240]">
                    {CHAT_TAB_MESSAGE[activeTab].messages.map((message, index)=>(
                        <div key={index}>
                            ...
                            {index < CHAT_TAB_MESSAGE[activeTab].messages.length-1 &&(<Separator/>)}
                        </div>
                    ))}
                </div>
            </div>
